...
if [ -z ${DEFI_HOME+x} ]; then
	...
	docker run -d --restart always --name defi_node -p 8554:8554 -p 8555:8555 -p 5000:5000 -p 5001:5001 -v "$FOLDER:/root/.defi" defi_chain defid -maxmempool=64 -dbcache=128 -rpcallowip=172.17.0.0/16 -rpcbind=0.0.0.0 -rpcuser='definode_user' -rpcpassword='definode_pw'
